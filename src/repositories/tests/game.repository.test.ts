...
describe('GameRepository', () => {
  ...
  it('should update a game', async () => {
    ...
    const mockOldGame: IGame = {
      ...
      createdAt: new Date(), 
      teams: mockGame.teams.map(team => ({members: [], teamId: team, score: 0 })),
  ...
  it('should delete a game', async () => {
    ...
    mockedGamesDb.get.mockResolvedValue({ _id: gameId, _rev: gameRev });
    mockedGamesDb.destroy.mockResolvedValue({ ok: true });
    await gameRepositoryX.delete(gameId);

    expect(mockedGamesDb.get).toHaveBeenCalledWith(gameId);
    expect(mockedGamesDb.destroy).toHaveBeenCalledWith(gameId, gameRev);
  });
});
